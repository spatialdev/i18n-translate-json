...
import { run as translate } from './translate';
...
// run translation
var run = function () {
  resolve(__dirname, startDir);

  translate(apiKey, startDir, sourceLang, targetLang, function (err, result) {
    if (err) {
      console.log('ERROR:');
      console.log(err);
      process.exit(0);
  ...
  getJSON(
    ...
    function (error, response) {
      if (error) {
        ...
        console.log(langs);
        console.log(
          'all these languages to choose from and you decided to use none?'
        );
        // run();
        process.exit(0);
      }
